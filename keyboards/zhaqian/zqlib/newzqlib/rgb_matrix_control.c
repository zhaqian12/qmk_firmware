...
rgb_matrix_control_config_t rgb_matrix_control_config;
...
void rgb_matrix_control_task(void) {
    for (uint8_t i = 0; i < DRIVER_LED_TOTAL; i ++) {
        if (HAS_FLAGS(g_led_config.flags[i], LED_FLAG_UNDERGLOW)) {
            if (rgb_matrix_control_config.is_underglow_rgb_enable == 0)
                rgb_matrix_set_color(i, 0x00, 0x00, 0x00);
        } else if (HAS_FLAGS(g_led_config.flags[i], LED_FLAG_INDICATOR)) {
            if (rgb_matrix_control_config.is_logo_rgb_enable == 0)
                rgb_matrix_set_color(i, 0x00, 0x00, 0x00);
        } else {
            if (rgb_matrix_control_config.is_key_rgb_enable == 0)
                rgb_matrix_set_color(i, 0x00, 0x00, 0x00);
        }
    }
}
